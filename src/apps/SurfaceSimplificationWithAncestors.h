...
//! \ingroup algorithms
class SurfaceSimplificationWithAncestors
{
public:
    //! Construct with mesh to be simplified.
    SurfaceSimplificationWithAncestors(SurfaceMesh& mesh);

    //! Destructor.
    ~SurfaceSimplificationWithAncestors();
...
private:
	...
	FaceProperty<std::vector<Face>> original_faces;
	VertexProperty<Vertex> original_verts;
