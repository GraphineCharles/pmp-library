...
// Modified by Cholleme version of standard "SurfaceSimplification" class that also keeps ancestors around
...
SurfaceSimplificationWithAncestors::SurfaceSimplificationWithAncestors(SurfaceMesh& mesh)
...
SurfaceSimplificationWithAncestors::~SurfaceSimplificationWithAncestors()
{
...
void SurfaceSimplificationWithAncestors::initialize(Scalar aspect_ratio, Scalar edge_length,
                                       ...
                                       Scalar hausdorff_error)
{
	...
	original_faces = mesh_.face_property<std::vector<Face>>("f:orignalFaces");
	for (auto f : mesh_.faces())
	{
		original_faces[f].clear();
		original_faces[f].push_back(f);
	}

	original_verts = mesh_.vertex_property<Vertex>("f:orignalVertices");
	for (auto v : mesh_.vertices())
	{
		original_verts[v] = v;
	}
...
void SurfaceSimplificationWithAncestors::simplify(unsigned int n_vertices)
{
    if (!mesh_.is_triangle_mesh())
    {
...
void SurfaceSimplificationWithAncestors::enqueue_vertex(Vertex v)
{
...
bool SurfaceSimplificationWithAncestors::is_collapse_legal(const CollapseData& cd)
{
...
float SurfaceSimplificationWithAncestors::priority(const CollapseData& cd)
{
...
void SurfaceSimplificationWithAncestors::postprocess_collapse(const CollapseData& cd)
{
	// update original lists
	Face remainLeft = mesh_.face(mesh_.opposite_halfedge(cd.vlv0));
	assert(remainLeft != cd.fl);
	Face remainRight = mesh_.face(mesh_.opposite_halfedge(cd.v0vr));
	assert(remainRight != cd.fr);

	original_faces[remainLeft].insert(original_faces[remainLeft].end(), original_faces[cd.fl].begin(), original_faces[cd.fl].end());
	original_faces[remainRight].insert(original_faces[remainRight].end(), original_faces[cd.fr].begin(), original_faces[cd.fr].end());
...
Scalar SurfaceSimplificationWithAncestors::aspect_ratio(Face f) const
{
...
Scalar SurfaceSimplificationWithAncestors::distance(Face f, const Point& p) const
{
    SurfaceMesh::VertexAroundFaceCirculator fvit = mesh_.vertices(f);
...
SurfaceSimplificationWithAncestors::CollapseData::CollapseData(SurfaceMesh& sm, Halfedge h)
    : mesh(sm)
{
    v0v1 = h;
